...
public class DnD_Joueur : IDragAndDrop
{
   private void Awake() 
    {
    ...
    private void OnTriggerEnter(Collider other) 
    {
        if(other.CompareTag("Position"))
        {
            if(other.transform.parent.gameObject == attributManager.listeObjet && other.transform.gameObject.activeSelf)
            {
                ...
                other.transform.gameObject.GetComponent<DnD_objet>().GetImage().color = new Color(); 
                other.transform.gameObject.GetComponent<DnD_objet>().GetImage().enabled = true;
